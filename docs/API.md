...
| `AUTH409` | 이미 가입된 이메일 |
| `USER409` | 닉네임 중복 |
| `AUTH400` | 비밀번호 형식 오류 (8자 이상, 영문+숫자+특수문자) |
...
| 에러 코드 | 상황 |
|---|---|
| `AUTH401_2` | 유효하지 않거나 만료된 인가코드 |
| `AUTH500` | 소셜 제공자 API 호출 실패 |
...
**한 사람은 한 판만 (#50)** — 이 엔드포인트는 호출될 때마다 그 유저의 **기존 활성 세션을 무효화**한다. 진행 중인 판이 있었다면 그 판은 즉시 버려지고(기록 저장 불가, 이후 제출은 `SOLO404`), 새 판만 유효하다.

- 서버가 지키는 불변식이다 — 프론트가 이전 판 취소를 요청하지 않아도 고아 세션이 생기지 않는다. 클라이언트에 정리를 맡기면 요청이 실패하거나 브라우저가 죽을 때(뒤로가기로 페이지를 떠나는 경로) 세션이 남는다
- 구현: `solo:user:{userId}`가 현재 판을 가리키고, 교체를 Lua(`GET` → `SET EX`)로 원자화한다. 연타로 요청이 겹쳐도 각자 서로 다른 직전 값을 받아 하나씩만 정리하므로 **살아남는 세션은 항상 포인터가 가리키는 하나**다
- 대가: **두 탭에서 동시에 플레이할 수 없다.** 솔로 게임은 "한 사람이 한 판"이 상식이라 받아들인 제약이다
- TTL(`solo:session:*` 10분)은 최후 방어선으로 남는다 — 서버 정리가 실패하는 경로(세션 저장과 포인터 교체 사이의 장애)가 있기 때문
...
| 에러 코드 | 상황 |
|---|---|
| `SOLO404` | 존재하지 않거나 만료된 게임 세션. **새 판을 시작해 무효가 된 판**도 여기 해당한다(#50) |
| `SOLO400` | 제출 기록 검증 실패 (합이 10이 아닌 move 포함 등) |
| `SOLO400_1` | 제한시간을 벗어난 기록 — `elapsedMs`가 120초(+1초 오차)를 넘거나 순서가 역순 (#49) |
| `SOLO409` | 이미 제출된 세션 (중복 제출 — 동시성 학습 포인트) |

**제출 창과 제한시간은 별개다 (#49)** — 세션 TTL 10분은 "언제까지 제출을 받아줄까"만 정하고, "제한시간을 지켰는가"는 `moves`의 `elapsedMs` 검증이 맡는다. 브라우저가 타이머를 멈춘 경우(뒤로가기·비활성 탭·화면 잠김) 제출이 수 분 늦어도 기록은 저장된다. 30초 넘게 늦은 제출은 서버가 INFO 로그로 관측한다.
...
| 에러 코드 | 상황 |
|---|---|
| `ROOM400` | 자기 방에는 입장할 수 없음 (host가 자기 방 코드로 join) |
| `ROOM404` | 존재하지 않는 방 |
| `ROOM409` | 방이 가득 참 (동시 입장 시 한 명만 성공해야 함) |
| `ROOM409_1` | 이미 게임이 진행 중인 방 |
...
Host                    Server                    Guest
 ...
 │                        │                         │
 │            ... 120초 경과 ...                     │
 │◄══ GAME_END (결과 + 누적 점수 + DB 정산 완료) ════►│
 │─── /ready (다음 판) ───►│◄──── /ready (다음 판) ───│   ← 같은 방에서 연전
 │◄══ GAME_START (round=2, 새 보드) ════════════════►│
```
